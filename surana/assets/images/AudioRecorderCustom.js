...
const AudioRecorderCustom = () => {
  const route = useRoute();
  const enteredText = route.params?.enteredText || 'Default Text'; // Use the entered text or a default value
  ...
  async function speak(text) {
    return Speech.speak(text, { language: 'en', rate: 0.5 });
  }
  ...
  async function stopRecording() {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      // Ask the next question using text-to-speech and start the countdown after speech
  ...
  return (
    <LinearGradient
      colors={['black', 'black', 'black','black', '#0C1B4F']}
      style={styles.gradientContainer}
    >
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.imageContainer}>
          <Image
            source={require('/Users/yugamsurana/Desktop/omg/language/src/components/images/night-city.gif')}
...
const styles = StyleSheet.create({
  ...
  textButton: {
    ...
    paddingVertical: 10,
    marginBottom: 0,
    backgroundColor: 'rgba(255, 255, 255, 0.1)', // Adjust the alpha (last value) as needed
    // borderColor: 'white',
    // borderWidth: 1
  },
  buttonContainer: {
...
export default AudioRecorderCustom;